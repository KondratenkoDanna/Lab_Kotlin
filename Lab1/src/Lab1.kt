...
fun otvet(answer: String) =
    when (answer) {
        ...
        "c" -> println("тебе может понравиться епта скрипт")
...
//________58________
tailrec fun spiral(index: Int, allKol: Double, kol: Double, step: Double): Double {

    var kolNew = kol
    if (simpleNumber(index + step.toInt()) == true)
        kolNew++

    if (simpleNumber(index + step.toInt() * 2) == true)
        kolNew++

    if (simpleNumber(index + step.toInt() * 3) == true)
        kolNew++

    if (simpleNumber(index + step.toInt() * 4) == true)
        kolNew++
    println("koln = $kolNew")
    println(kol/allKol)
    if (kol / allKol < 0.65) return step else

        return spiral(index, allKol + 4, kolNew, step + 2)
}

fun main()
{
    print(spiral(1,0.0,0.0,2.0))
}